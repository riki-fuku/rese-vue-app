<template>
    ...
    <v-main color="gray-lighten-2">
        <v-container fluid width="100%" v-if="loading">
            <p class="pa-5 text-h5 font-weight-bold">店舗情報変更</p>
...
const resetShop = async () => {
    ...
    name.value = shopUserWithShop.value.name
    name.error_message = ''
    areaId.value = shopUserWithShop.value.area_id
    areaId.error_message = ''
    genreId.value = shopUserWithShop.value.genre_id
    genreId.error_message = ''
    description.value = shopUserWithShop.value.description
    description.error_message = ''
    openingTime.value = shopUserWithShop.value.opening_time.substring(0, 5)
    openingTime.error_message = ''
    closingTime.value = shopUserWithShop.value.closing_time.substring(0, 5)
    closingTime.error_message = ''
    topImage.value = ''
    topImage.error_message = ''
}
...
const updateShop = async () => {
    ...
    if (validateShop()) {
        try {
            ...
            formData.append('id', shopId.value);
            ...
            const response = await axios.post(import.meta.env.VITE_API_URL + '/shop/update', formData, {
                headers: {
...
onMounted(async () => {
    ...
    shopId.value = shopUserWithShop.value.id
    name.value = shopUserWithShop.value.name
    areaId.value = shopUserWithShop.value.area_id
    genreId.value = shopUserWithShop.value.genre_id
    description.value = shopUserWithShop.value.description
